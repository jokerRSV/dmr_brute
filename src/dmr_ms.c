    ...
    for (j = 0; j < 6; j++) {
        ...
        if (state->dmr_ms_rc == 0) {
            if (state->directmode == 0) {
                ...
                processMbeFrame(state, ambe_fr4); //play duplicate of 2 here to smooth audio on tdma direct
                processMbeFrame(state, ambe_fr2);
                processMbeFrame(state, ambe_fr3);
            }
        }
...
void dmrMSBootstrap(dsd_opts *opts, dsd_state *state) {
    ...
    if (state->directmode == 0) {
        ...
        processMbeFrame(state, ambe_fr4); //play duplicate of 2 here to smooth audio
        processMbeFrame(state, ambe_fr2);
        processMbeFrame(state, ambe_fr3);
    }
