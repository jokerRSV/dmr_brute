        ...
        switch (DMR_PDU[i]) {
            ...
            case 0x55: //circle-3d
                if (message_len > 0 && lat == 0) {
                    ...
                    i += 10;
                    if (lat > 0 && lon > 0)
                        lrrp_confidence++; //would be better to set by absolute boundary (180 and 90?)
                    else lat = 0;
                }
                break;
            ...
            case 0x6C: //speed-hor
                if (message_len > 0 && vel_set == 0) {
                    vel = (DMR_PDU[i + 1] << 8) +
                          DMR_PDU[i + 2]; //don't think this is meant to be shifted, but doing it anyways for comparison
    ...
    if (report && message_len > 0) {
        ...
        if (lrrp_confidence >= 3) //find the sweet magical number
        {
            ...
            if (opts->lrrp_file_output == 1) {
                ...
                if (!year)
                    fprintf(pFile, "%s\t",
                            getDateL()); //current date, only add this IF no included timestamp in LRRP data?
                if (!year)
                    fprintf(pFile, "%s\t",
                            getTimeL()); //current timestamp, only add this IF no included timestamp in LRRP data?
                if (year)
            ...
            if (report) {
                ...
                if (report == 0x1F)
                    fprintf(stderr, " ARRP_TriggeredInformationReport_NCDT "); //customize later when more is learned
            ...
            if (vel_set) {
                fprintf(stderr, "\n");
                fprintf(stderr, "  LRRP - Velocity: %.4lf m/s %.4lf km/h %.4lf mph", velocity, (3.6 * velocity),
                        (2.2369 * velocity));
...
void dmr_locn(dsd_opts *opts, dsd_state *state, uint8_t block_len, uint8_t DMR_PDU[]) {
    ...
    for (i = 0; i < ((blocks * block_len) - (padding + 4)); i++) {
        switch (DMR_PDU[i]) {
            ...
            case 0x45: //E -- East
                lon = 1;
                lon_deg = ((DMR_PDU[i + 1] - 0x30) << 8) | ((DMR_PDU[i + 2] - 0x30) << 4) |
                          ((DMR_PDU[i + 3] - 0x30) << 0);
    ...
    if (lat && lon) {
        ...
        fprintf(stderr, "Lat: %s %02X%s%02X\"%04X' Lon: %s %02X%s%02X\"%04X' ", lat_ord, lat_deg, deg_glyph, lat_min,
                lat_sec, lon_ord, lon_deg, deg_glyph, lon_min, lon_sec);
