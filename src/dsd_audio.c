...
void openPulseOutput(dsd_opts *opts) {
    ...
    if (opts->monitor_input_audio == 1) {
        opts->pulse_raw_dev_out = pa_simple_new(NULL, "DSD-FME3", PA_STREAM_PLAYBACK, NULL, "Raw Audio Out", &ss, NULL,
                                                NULL, NULL);
    }
...
//tt
    ...
    if (opts->dmr_stereo == 0) {
        opts->pulse_digi_dev_out = pa_simple_new(NULL, "DSD-FME", PA_STREAM_PLAYBACK, NULL, opts->output_name, &tt,
                                                 left, NULL, NULL);

    }

    if (opts->dmr_stereo == 1) {
        opts->pulse_digi_dev_out = pa_simple_new(NULL, "DSD-FME1", PA_STREAM_PLAYBACK, NULL, "XDMA SLOT 1", &tt, left,
                                                 NULL, NULL);
        opts->pulse_digi_dev_outR = pa_simple_new(NULL, "DSD-FME2", PA_STREAM_PLAYBACK, NULL, "XDMA SLOT 2", &tt, right,
                                                  NULL, NULL);
    }
...
void openPulseInput(dsd_opts *opts) {
    ...
    opts->pulse_digi_dev_in = pa_simple_new(NULL, "DSD-FME", PA_STREAM_RECORD, NULL, opts->output_name, &cc, NULL, NULL,
                                            NULL);
...
processAudio(dsd_opts *opts, dsd_state *state) {
    ...
    if (opts->audio_gain >= 0) {
        ...
        for (n = 0; n < 160; n++) {
            *state->audio_out_temp_buf_p =
                    (state->aout_gain + ((float) n * gaindelta)) * (*state->audio_out_temp_buf_p);
            state->audio_out_temp_buf_p++;
        }
    ...
    if (opts->pulse_digi_rate_out > 8000) {
        ...
        state->audio_out_float_buf_p -= (960);
...
processAudioR(dsd_opts *opts, dsd_state *state) {
    ...
    if (opts->pulse_digi_rate_out > 8000) {
        ...
        state->audio_out_float_buf_pR -= (960);
        // copy to output (short) buffer
        for (n = 0; n < 960; n++) {
...
playSynthesizedVoiceR(dsd_opts *opts, dsd_state *state) {
    ...
    if (state->audio_out_idxR > opts->delay) {
        ...
        {
            //OSS
            result = write(opts->audio_out_fdR, (state->audio_out_buf_pR - state->audio_out_idxR),
